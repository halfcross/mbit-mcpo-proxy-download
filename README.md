...
**Recommended** — one-line install (download, copy to Applications, clear quarantine):
...
**Alternative:** download the **`.dmg`** from [Releases](https://github.com/halfcross/mbit-mcpo-proxy-download/releases) or the [GitHub Pages site](https://halfcross.github.io/mbit-mcpo-proxy-download/), open it, drag **MindBehindAI** to **Applications**, then follow **[quarantine troubleshooting](https://halfcross.github.io/mbit-mcpo-proxy-download/guide/help/troubleshooting.html#macos-quarantine)** if macOS blocks the app.
...
**Known on Windows:** builds are **not code-signed** (SmartScreen may ask **More info → Run anyway**). The first start can be **slow** while the portable bundle unpacks and **Microsoft Defender** scans it. Optional speed-up: add Defender exclusions for the `.exe`, `%TEMP%\MindBehindAI`, and `%APPDATA%\MindBehind\mcp-orchestrator` — step-by-step in the **[user guide → Troubleshooting → Windows](https://halfcross.github.io/mbit-mcpo-proxy-download/guide/help/troubleshooting.html#windows)**.
