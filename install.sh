#!/usr/bin/env bash
# MindBehindAI — macOS one-line installer
...
# Clear attrs on real files/dirs with `xattr -c`, and on symlink inodes with `xattr -hc` (do not follow).
clear_bundle_quarantine() {
  local app_root="$1"
  find "$app_root" \( -type f -o -type d \) -exec xattr -c {} + 2>/dev/null || true
  find "$app_root" -type l -exec xattr -hc {} + 2>/dev/null || true
}
