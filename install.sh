...
echo ""
echo " This will:"
echo "  1. Download the latest release DMG from GitHub (progress below)"
...
# Best-effort: show expected size before download (GitHub usually sends Content-Length on the asset URL).
if REMOTE_LINE=$(
  curl -fsSIL "$DMG_URL" 2>/dev/null | tr -d '\r' | grep -i '^content-length:' | head -1 | awk '{print $2}'
); then
  if [[ -n "${REMOTE_LINE:-}" && "$REMOTE_LINE" =~ ^[0-9]+$ ]]; then
    MB_TOTAL=$(/usr/bin/python3 -c "print(f'{int('$REMOTE_LINE') / 1024 / 1024:.1f}')")
    echo ""
    echo " Expected size (from server): ~${MB_TOTAL} MB  (${REMOTE_LINE} bytes)"
  fi
fi
...
echo " (Progress: curl prints %, bytes, total size, average speed, and ETA when stderr is a terminal.)"
echo ""

# No -s: stderr shows curl’s full progress meter (more detail than --progress-bar).
if [[ -t 2 ]]; then
  curl -fL -o "$DMG_PATH" "$DMG_URL"
else
  # Piped / non-TTY: still avoid silent failure; print a simple line after
  curl -fSL -o "$DMG_PATH" "$DMG_URL"
  echo " Download finished (non-interactive mode — no live progress)."
fi

if [[ -f "$DMG_PATH" ]]; then
  GOT=$(stat -f%z "$DMG_PATH" 2>/dev/null || echo "")
  if [[ -n "$GOT" ]]; then
    MB_GOT=$(/usr/bin/python3 -c "print(f'{int('$GOT') / 1024 / 1024:.2f}')")
    echo ""
    echo " Saved: $(basename "$DMG_PATH")  (~${MB_GOT} MB on disk)"
  fi
fi
